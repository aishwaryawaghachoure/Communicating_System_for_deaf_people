	...
	while True:
		...
		while result.upper()!="STOP":
			...
			if(result.upper()!="STOP"):
				...
				print((finalString[:len(finalString)-4]))
				socket.send(bytes(finalString[:len(finalString)-4]))
				texttospeech(finalString[:len(finalString)-4])
				flag=1;
